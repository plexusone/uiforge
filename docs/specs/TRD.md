...
- **Modes & density.** `ThemeRef.modes` holds per-mode token overlays (derived from DSS per-token values by `theme.FromDesignSystemWithModes`); `variant` names the default and renderers switch at runtime (`mode` prop/property, `data-uiforge-mode` attribute, `CSSWithModes` for stylesheet theming). `ThemeRef.density` (`comfortable`/`compact`) surfaces as `data-uiforge-density` plus a `--uiforge-density` scale factor consumed by pack spacing. First-class DSS support is proposed upstream (docs/proposals/dss-density-and-modes.md).

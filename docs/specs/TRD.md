...
- **Modes & density.** `ThemeRef.modes` holds per-mode token overlays, derived by `theme.FromDesignSystemWithModes` from DSS's first-class `Modes` declaration and `ColorToken.EffectiveModes()` (DSS v0.7.0, [systemspec-designsystem#9](https://github.com/plexusone/systemspec-designsystem/issues/9)); `variant` names the default and renderers switch at runtime (`mode` prop/property, `data-uiforge-mode` attribute, `CSSWithModes` for stylesheet theming). `ThemeRef.density` (`comfortable`/`compact`) surfaces as `data-uiforge-density` plus a `--uiforge-density` scale factor consumed by pack spacing — this remains UIForge's own model; first-class DSS density support is still proposed upstream (docs/proposals/dss-density-and-modes.md).

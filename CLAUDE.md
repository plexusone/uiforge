...
- **`spec/` (`@plexusone/uiforge-spec`) is the single source for the UISpec TS types and the framework-free engines** (expression, state, interaction, datasource, data). Both renderers depend on it via `file:../../spec` — build `spec/` before building a renderer. The `file:` links are rewritten to semver dependencies at publish time by `scripts/npm-publish.sh` (never committed); the packages are published on npm (publish with `NPM_CLIENT=pnpm scripts/npm-publish.sh`). Both renderers ship parallel core/analytics/application component packs that must emit identical DOM.
...
- The React renderer exposes `createExternalStoreRuntime` (renamed from its historical AgentOS-specific name in RMI-UIFORGE-115) as the neutral external-store conversation runtime adapter.

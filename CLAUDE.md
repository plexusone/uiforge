...
- **`spec/` (`@plexusone/uiforge-spec`) is the single source for the UISpec TS types and the framework-free engines** (expression, state, interaction, datasource, data). Both renderers depend on it via `file:../../spec` — build `spec/` before building a renderer. The `file:` links are rewritten to semver dependencies at publish time by `scripts/npm-publish.sh` (never committed); until packages are on npm, consuming a renderer via git requires the whole repo. Both renderers ship parallel core/analytics/application component packs that must emit identical DOM.

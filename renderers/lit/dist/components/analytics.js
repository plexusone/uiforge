...
function primaryData(instance, ctx) {
...
function primaryStatus(instance, ctx) {
...
export function renderMetric(instance, ctx) {
...
export function renderTable(instance, ctx) {
    const status = primaryStatus(instance, ctx);
    const data = primaryData(instance, ctx);
...
const CHART_W = 300;
const CHART_H = 120;
export function renderLineChart(instance, ctx) {
    const status = primaryStatus(instance, ctx);
    const points = extractSeries(primaryData(instance, ctx), prop(instance, 'xAxis', ''), prop(instance, 'yAxis', ''));
    return html `
    <div style=${styleMap(cardStyle)} data-uiforge-component=${instance.id}>
      <div style=${styleMap(titleStyle)}>${prop(instance, 'title', '')}</div>
...
export function renderBarChart(instance, ctx) {
    const status = primaryStatus(instance, ctx);
    const points = extractSeries(primaryData(instance, ctx), prop(instance, 'xAxis', ''), prop(instance, 'yAxis', ''));
    const onClick = (p) => () => ctx?.dispatch(instance.id, 'click', { label: p.label, value: p.value });
...
export function renderGauge(instance, ctx) {
    const status = primaryStatus(instance, ctx);
    const data = primaryData(instance, ctx);

...
function prop(instance, key, fallback) {
...
export function renderInput(instance, ctx) {
...
export function renderSelect(instance, ctx) {
    ...
    <label data-uiforge-component=${instance.id} style=${styleMap(fieldStyle)}>
      ...
      >
        ${options.map((opt) => html `<option value=${String(opt)} ?selected=${String(opt) === String(current)}>
              ${String(opt)}
            </option>`)}
      </select>
    </label>
...
export function renderCheckbox(instance, ctx) {
...
export function renderForm(instance, ctx, children) {
    const style = {
        display: 'flex',
        flexDirection: 'column',
...
export function renderRecordDetail(instance, ctx) {
    ...
    const declared = prop(instance, 'fields', []);
...
export function renderRecordList(instance, ctx) {
    ...
    const declared = prop(instance, 'columns', []);
    const columns = declared.length > 0 ? declared : rows.length > 0 ? Object.keys(rows[0]) : [];
    const selectable = prop(instance, 'selectable', false);
    const title = prop(instance, 'title', '');
    const cellStyle = 'padding: 4px 8px; border-bottom: 1px solid var(--uiforge-border, #e2e8f0); text-align: left';
    return html `
    <div
      data-uiforge-component=${instance.id}
      style="font-family: var(--uiforge-font-family, system-ui, sans-serif)"
...
export function renderActionBar(instance, ctx) {
    const actions = prop(instance, 'actions', []);
    const alignment = prop(instance, 'alignment', 'start');
    const style = {
        display: 'flex',
        gap: '8px',
...
export function renderBadge(instance) {
